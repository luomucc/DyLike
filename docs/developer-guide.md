...
短视频播放支持基于用户行为的智能推荐排序，实现"探索-利用"平衡策略。

### 核心文件

- 推荐排序工具：`dy-player/src/main/java/me/lingci/dy/player/util/RecommendSorter.kt`
- 短视频播放页：`dy-player/src/main/java/me/lingci/dy/player/ui/short_video/ShortVideoActivity.kt`
- 视频数据实体：`dy-player/src/main/java/me/lingci/dy/player/entity/VideoData.kt`

### 评分机制

采用多维度加权评分，权重配置如下：

- **点赞**（10分）：最高优先级，7天内点赞额外加分，越近越高
- **播放次数**（3分/次，上限10次）：反复播放更能说明喜好
- **播放时长**（2分封顶）：使用对数映射，避免长视频天然优势
  - 3秒 → 0.4分
  - 10秒 → 0.8分
  - 30秒 → 1.2分
  - 60秒及以上 → 2分封顶
...
为了解决"未播放视频永远排在底部"的问题，算法采用混合策略：

1. **已播放视频**（利用）：按喜好度排序后分组打乱，保证高偏好内容优先展示
2. **未播放视频**（探索）：赋予20-35分的稳定探索分（基于文件名哈希），均匀插入推荐列表
3. **混合规则**：每2个已播放组插入1个未播放视频，确保新内容有机会被看到

### 调用方式

```kotlin
// 在 ShortVideoActivity.initData() 中调用
val sortedList = RecommendSorter.smartShuffle(videoList)
```

### 设计原则

- 短视视频完整看完也能获得合理高分，不会被长视频压制
- 播放次数比单次时长更能反映真实喜好
- 未播放内容均匀分布，避免信息茧房
- 同一视频的探索分稳定，避免每次排序结果跳动
