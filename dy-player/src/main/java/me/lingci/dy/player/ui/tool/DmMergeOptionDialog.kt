 ...
 * version: 1.0
 */
class DmMergeOptionDialog : DialogFragment() {
    ...
    override fun onStart() {
        ...
        arguments?.let { bundle ->
            ...
            binding.tvOpHint.text = resources.getString(R.string.hint_dm_merge_show, op.toString())
            binding.tvEdHint.text = resources.getString(R.string.hint_dm_merge_top, ed.toString())
    ...
    private fun init() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            binding.opSeekBar.min = 2
        }
        binding.opSeekBar.max = 20
        binding.opSeekBar.setOnSeekBarChangeListener(object : OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                binding.tvOpHint.text = resources.getString(R.string.hint_dm_merge_show, progress.toString())
            }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            binding.edSeekBar.min = 5
        }
        binding.edSeekBar.max = 50
        binding.edSeekBar.setOnSeekBarChangeListener(object : OnSeekBarChangeListener{
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                binding.tvEdHint.text = resources.getString(R.string.hint_dm_merge_top, progress.toString())
